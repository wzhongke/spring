...
//         }
    ...
    public static String queryFormat(String str){
        ...
        while (m.find()) {
            try {
                //m.appendReplacement(sb, URLDecoder.decode(unicodeTrans(m.group()),"unicode"));
    ...
    public static String unicodeTrans(String str){
        str = str.substring(2, str.length() - 1);
        int tmp = 0;
        try{
            tmp = Integer.parseInt(str);
        }catch(NumberFormatException e){
            System.out.println("query unicode 反编译异常！");
        }
        str = Integer.toHexString(tmp);
        StringBuffer sb = new StringBuffer();
        sb.append("%FE%FF");
        if(str.length()%2 == 1){
            str = "0" + str;
